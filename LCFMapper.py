...
def scanDirs(p_sFile, p_sRootFolder, p_sSourceImgDir, p_AcceptedFormatS = (".XML",)):
    """
    ...
    try:
        for f in listdir(p_sFile):
            try:
                src = os.path.join(p_sFile, f)
                # if it's NOT a directory
                if not os.path.isdir(src):
                    if os.path.splitext(os.path.basename(f))[1].upper() in p_AcceptedFormatS:
                        sf = SourceXML(src, p_sRootFolder)
                        ...
                        if os.path.splitext(os.path.basename(f))[0].upper() not in SourceImage.source_pict_dict:
                            sI = SourceImage(os.path.relpath(src, p_sRootFolder), root=p_sRootFolder)
                            if p_sSourceImgDir in sI.fullDirName and p_sSourceImgDir:
                                sI.isEncodedImage = True
                            SourceImage.source_pict_dict[sI.fileNameWithExt.upper()] = sI
                else:
                    scanDirs(src, p_sRootFolder, p_sSourceImgDir)
...
def main2():
    ...
    targGDLDir = r"C:\Users\samu.karli\Desktop\Uj mappa\GDL"     # TargetGDLDirName.get()
    targPicDir = r"C:\Users\samu.karli\Desktop\Uj mappa\IMG"     # TargetImageDirName.get()   # For target library's encoded images
    ...
    pool_map = [{"dest": dest_dict[k],
                 "tempdir": tempdir,
                 "bOverWrite": False,                               # bOverWrite.get(),
                 "pict_dict": pict_dict,
                 "dest_dict": dest_dict,
    ...
    SourceImageDirName = r"C:\Users\samu.karli\Desktop\Archicad Library 26.gsm\IMG"
    ACLocation = strHack(r"C:\Program Files\GRAPHISOFT\Archicad 26")
    for f in list(pict_dict.keys()):
        if pict_dict[f].sourceFile.isEncodedImage:
            ...
            if targPicDir:
                try:
                    shutil.copyfile(os.path.join(SourceImageDirName, pict_dict[f].sourceFile.relPath),
